...
// This method writes the config files and starts the launch thread.
{
    ...
    if (!gameIsRunning && !gameProcessAlreadyRuns) { // the game is not already running
        ...
        [fileContents appendString:@"seta g_autoMapCycle 1\n"];
        [fileContents appendString:@"set g_forceBasedTeams 1\n"];
        ...
        if ([isOnlineServer intValue] == 1) { // yes, online server
            [fileContents appendString:[NSString stringWithFormat:@"seta ui_gamespy %d\n", [setGameSpy intValue]]];
            ...
            [fileContents appendString:[NSString stringWithFormat:@"seta sv_master4 \"%@\"\n", [onlineServer4 stringValue]]];
        } else { // LAN server
            [fileContents appendString:@"seta ui_gamespy 0\n"];
        ...
        // server tab items
        [fileContents appendString:[NSString stringWithFormat:@"seta g_respawnInvulnerability %d\n", [setInvulerabilityTime intValue]]];
        ...
        [fileContents appendString:[NSString stringWithFormat:@"seta g_teamAutoJoin %d\n", [setAutoJoin intValue]]];
        ...
        [fileContents appendString:[NSString stringWithFormat:@"seta g_privateDuel %d\n", [setPrivateDuels intValue]]];
        [fileContents appendString:[NSString stringWithFormat:@"seta g_saberLocking %d\n", [setSaberLocking intValue]]];
        [fileContents appendString:[NSString stringWithFormat:@"seta g_allowDuelSuicide %d\n", [setAllowSuicide intValue]]];
        ...
        // weapons tab items
        int weapons = 0;
        if (![enableWeapon1 state]) { weapons += [enableWeapon1 tag]; }
        if (![enableWeapon2 state]) { weapons += [enableWeapon2 tag]; }
        if (![enableWeapon3 state]) { weapons += [enableWeapon3 tag]; }
        if (![enableWeapon4 state]) { weapons += [enableWeapon4 tag]; }
        if (![enableWeapon5 state]) { weapons += [enableWeapon5 tag]; }
        if (![enableWeapon6 state]) { weapons += [enableWeapon6 tag]; }
        if (![enableWeapon7 state]) { weapons += [enableWeapon7 tag]; }
        if (![enableWeapon8 state]) { weapons += [enableWeapon8 tag]; }
        if (![enableWeapon9 state]) { weapons += [enableWeapon9 tag]; }
        if (![enableWeapon10 state]) { weapons += [enableWeapon10 tag]; }
        if (![enableWeapon11 state]) { weapons += [enableWeapon11 tag]; }
        if (![enableWeapon12 state]) { weapons += [enableWeapon12 tag]; }
        if (![enableWeapon13 state]) { weapons += [enableWeapon13 tag]; }
        
        int powers = 0;
        if (![enablePower1 state]) { powers += [enablePower1 tag]; }
        if (![enablePower2 state]) { powers += [enablePower2 tag]; }
        if (![enablePower3 state]) { powers += [enablePower3 tag]; }
        if (![enablePower4 state]) { powers += [enablePower4 tag]; }
        if (![enablePower5 state]) { powers += [enablePower5 tag]; }
        if (![enablePower6 state]) { powers += [enablePower6 tag]; }
        if (![enablePower7 state]) { powers += [enablePower7 tag]; }
        if (![enablePower8 state]) { powers += [enablePower8 tag]; }
        if (![enablePower9 state]) { powers += [enablePower9 tag]; }
        if (![enablePower10 state]) { powers += [enablePower10 tag]; }
        if (![enablePower11 state]) { powers += [enablePower11 tag]; }
        if (![enablePower12 state]) { powers += [enablePower12 tag]; }
        if (![enablePower13 state]) { powers += [enablePower13 tag]; }
        if (![enablePower14 state]) { powers += [enablePower14 tag]; }
        if (![enablePower15 state]) { powers += [enablePower15 tag]; }
        if (![enablePower16 state]) { powers += [enablePower16 tag]; }
        if (![enablePower17 state]) { powers += [enablePower17 tag]; }
        if (![enablePower18 state]) { powers += [enablePower18 tag]; }
        
        // NSLog(@"weapons: %d", weapons);
        // NSLog(@"powers: %d", powers);
        
        [fileContents appendString:@"set g_duelWeaponDisable 65531\n"];
        [fileContents appendString:[NSString stringWithFormat:@"set g_WeaponDisable %d\n", weapons]];
        [fileContents appendString:[NSString stringWithFormat:@"set g_forcePowerDisable %d\n", powers]];
    ...
    if (configSaved) { // file is saved OKAY! launch the server
        ...
        [arguments addObject:[NSString stringWithFormat:@"+set net_port %d", [setNetPort intValue]]];

        // see if it's an dedicated ONLINE server
        if ([isOnlineServer intValue] == 1) { // yes, online
            [arguments addObject:GAMEPARAMETER3];
        } else { // no LAN only
                ...
                if (c >= (start+3)) { // get player name
                    ...
                    if ([currentLine length] == STATUSLINELEN) { // look for a bot
                        if ([[currentLine substringWithRange:NSMakeRange(57, 3)] isEqualToString:@"bot"]) { // it is a bot
                            [aPlayer addObject:@"bot"]; // ping
                            botCounter++;
                        } else {
                            [aPlayer addObject:[currentLine substringWithRange:NSMakeRange(10, 4)]]; // ping
                        }
                    } else {
                        [aPlayer addObject:[currentLine substringWithRange:NSMakeRange(10, 4)]]; // ping
                    }
